...
export default function Card({ title, url, icon, description }) {
    ...
    return <>
        ...
        <Link
            ...
            passHref
            target="_blank"
            title={description.length > 1 ? `${title} | ${description}` : title}
            className="sm:w-40 mx-auto mb-5 sm:mb-6 font-medium hover:underline hover:underline-offset-2 duration-500 bg-white rounded-3xl group"
            aria-label={`Click this tile to access ${title}`}>
